...
const mockLocation = {
  pathname: '/details/Cyprus says to strip passports from Turk Cypriot officials - Reuters$2021-08-23T13:04:00Z',
  hash: '',
  search: '',
  state: {
...
test('articles reducer should handle a APi data being added to the redux state', () => {
    const initialState = {
        dataApi: null,
        articles: [],
        loading: false,
...
describe('articles have all details', () => {
    ...
    it('articles section has pagination with buttons and input articles / page number', () => {
      const history = createMemoryHistory();
    ...
    it('article details is open as a separate page once clicked on article', async () => {
      const history = createMemoryHistory();
      ...
      history.push('Cyprus says to strip passports from Turk Cypriot officials - Reuters$2021-08-23T13:04:00Z');
        
        expect(history.location.pathname).toBe('/details/Cyprus says to strip passports from Turk Cypriot officials - Reuters$2021-08-23T13:04:00Z');  
        const articleTxt = await findByText('Cyprus says to strip passports from Turk Cypriot officials - Reuters')
